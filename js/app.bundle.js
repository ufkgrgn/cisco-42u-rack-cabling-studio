  ...
  function renderAllCables() {
    ...
    STATE.cables.forEach((cable) => {
      ...
      path.addEventListener('mouseenter', (e) => {
        const devA = activeRack.devices.find(d => d.instanceId === cable.from.instanceId);
        const devB = activeRack.devices.find(d => d.instanceId === cable.to.instanceId);
        if (!devA || !devB) return;
        const catA = HARDWARE_CATALOG[devA.catalogKey];
        const catB = HARDWARE_CATALOG[devB.catalogKey];
        const pA = catA ? catA.ports.find(p => p.id === cable.from.portId) : null;
        const pB = catB ? catB.ports.find(p => p.id === cable.to.portId) : null;
        ...
        dom.tooltip.innerHTML = `
          ...
          <span style="color:${cable.color};">&#9632;</span> ${escapeHtml(catA.modelTag)} [${escapeHtml(pA ? pA.name : '')}] &harr; ${escapeHtml(catB.modelTag)} [${escapeHtml(pB ? pB.name : '')}]
        `;
      });
...
`;
  ...
  window.RackStudio = { STATE, catalog:HARDWARE_CATALOG, getActiveRack, refresh, fit:fitRackToScreen, mountDeviceAt, loadCustomTopology, validateTopology, exportJson, switchActiveRack, addNewRack, removeDevice };
